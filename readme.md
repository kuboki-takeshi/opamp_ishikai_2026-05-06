...
> - 大きな誤解はなさそうですが、個々の要素回路とそれらを組み合わせた際の整合がやや取れていないように見えました。一方、時間をかけて検証するべき点としては、回路の定数決定の際に1回だけではなく何度かループさせて検証することをおすすめしたいと思います。今回と採用した設計手法はすべてのトランジスタが想定した動作店で動作する際にシステマティックに定数が決まるというものですが、実際にはDC, AC, TRANのシミュレーション結果を見て、いったん決めた定数を微調整する必要があります。回路設計の際の仕様（とりあえず動くものとはいえ利得や帯域などの目標値）や設計回路をシミュレーション(または実測も）で評価するテストベンチが定まっていると、電気特性の不審な点に気づくこともあります。
>
>     - 現在の設定ではカレントミラーのMOSは線形領域で動作している可能性があります。重要なのは各段に必要な電流をどのように見積もるのかであり、その電流を分配するのがこのバイアス電流とカレントミラーです。この時に必要な電流は、設計するオペアンプで想定するSRと負荷容量（＋できれば仮置きした位相補償容量）の積で求まる電流が2段目の最低限必要な電流となります。同様に差動増幅段についても、SRと位相補償容量と2段目CSのゲート容量の和との積で1段目差動増幅段の最低限必要な電流になると考えられます。
>
>     - これは悩ましいところです。今回のように"とりあえずなんとか動くもの"であれば、私なら全部同じオーバードライブ電圧、できれば0.2Vに統一して設計すると思います。こうしておくとgmがIDの10倍として近似できるためです。これで一旦設計して飽和領域で動かないときには0.1Vで検討すると思いますが、電源電圧に十分余裕があるためおそらく0.2Vで問題ないかと思います。オーバードライブ電圧を下げるのは低消費電力動作というよりは低電源電圧ですべてのトランジスタを飽和領域で動かすための方策の一つと考えます。
...
> - 回路図の並びでレイアウトする問題や課題が起こりがちですが、フロアプランを予め検討しておくとよいです。
>     - 良くはないのですが今回はそれほど問題ないと思います。対策としてはフロアプラン（どの場所に何を配置するのか、マッチングや配線長を含めて検討して計画する）の策定が挙げられます。
>     - 状況によりますが、基本的には囲むべきと考えます。基盤/WELLの電位を安定させるためにも基本的には必要と考えた方がよいと思います。
>     - 設計者にとってプロセスに対する信頼度に依存しますが、今回の場合なら私ならGR~DB>CSの順番です。

- 次は、高周波信号に対応したOPAMPを作ってみたいです。Lを小さくする必要があると思いますが、その際の設計で注意すべきことは何ですか？また、1umプロセスだと、どのくらいの周波数にまで対応できるでしょうか？

> - 負荷・構成・消費電力などの条件に依存しますが、同様の条件であれば500kHz～1MHz程度は達成できるのではないでしょうか。
...
> Lを最小寸法にせず設計したのはGood。λが抑えられます。

> 一方でパラメータを求める際にVds=2.5Vとなっており、実際にこの回路でかかる電圧は3-4V程度（たぶん電源電圧5Vと思われます）ではないかと考えられるため、実際のトランジスタの状態とは異なる場合もあることは留意いただければと思います。プッシュプルなら妥当ですが本構成では動作点がずれる可能性があります。
...
> 後にある各段の設計でもありますが、カレントミラーのゲート電位Vg(Ibの端子電位)=3.5V想定では、Vgs=5.0-3.5=1.5V程度、Vds=3.5Vになるような条件で求める必要がありますが、当該の条件とは異なっており、想定した動作状態になっているかをよく確認する必要があります。λも大きそうなのでちょっと心配。
...
> 図面にはありませんがテストベンチでは100pFの負荷で求めています。後の比較のため注釈。
...
> SR=5V/0.5us=10V/usになります。これはこの回路が許容できる最大の波形の傾きであり、信号の周期ではありません。そのため歪まない最大の正弦波周波数は約1.59MHzになります。（振幅1V=2Vppのとき）

> 正弦波のSRは次の通り
> 信号振幅A（このときVppは2A）、周波数fのとき、時間波形V(t)は下記の通り。SRはその時間微分。SRが最大になるのはt=0のとき。
```math
V(t) = A \sin \omega t = A \sin (2 \pi ft) , 
...
> その通りですが、この回路ではVbを直接変更できるので、これを電圧制御に変更して3.5Vと設定すると想定通りに動く可能性があります。
...
> 1.47Vを見つけることができませんでしたが、Vbが変わってオーバードライブが変わったため、L:1umから3umに変更が要因として考えられます。
...
> 少し大きいかも。。これは想定する負荷容量に依存しますが、ディスクリートピンで10～数十pF、高速対応だと5pFくらいだったと思うので、この値だと負荷容量と同じ程度で効果が薄くなる可能性があります。逆に想定負荷が100pFだと少ないかもしれません。零点を上げるためにはCcを下げるか、Rcを上げる必要があります。
...
> 要注意点。テストベンチの負荷が1Fになっているように見えます。
...
> MOSが90度回転しているところがありますが、ばらつき抑制のためにはMOSのゲートは単一方向にそろえた方がよいです。

その際、囲えそうなところはガードリングで囲ってみましたが、囲うべきところとそうでないところの区別は曖昧です。
> 状況次第ですが、タップも併せて確保できるので、基本的には囲むべきと思います。
...
> ダブルビアはOK。プロセスに対する信頼によりますが、できるならやった方がいいです。また大電流が想定されるときにはダブルと言わずたくさん打つ方がよいです。
> おおまかに～10Ω/個
...
> M6のゲートバイアス廃線と思われます。良くはないですが今回は許容できると思います。電流制御だとまずいですが、MOSのカレントミラーは電圧制御なので、このノードの電位が動かないようにすることが重要です。したがって長い配線は許容可能と考えました。ただし、信号配線と交差する箇所には配線間容量が生じ、特に高速に電位が変化するときに容量を通してこのノードの電位が変動する可能性があります。
...
> この結果からは2段目のドレインは約4.6Vと想定され、設計時想定の条件と異なっている可能性があります。
...
> 鏡像のようになっている点からはバイアス電流が想定と異なりカレントミラーや負荷が想定と違った、テストベンチの設定（具体的に抵抗の値）がよくなかった点も考えられます。1つ目はibの端子を電圧制御に変更することで特性改善するかを確認できます。2つ目については帰還抵抗があまりに小さいとオペアンプから駆動できる電流の上限に達してしまい正常に動作しないのが原因のため、抵抗値を数kΩとすると動作する可能性があります。
